...
type VerifyVCProofResponse =
  | VerifyVCProofResponseSuccess
  | VerifyVCProofResponseFailure;
...
const verifyVCProof: VerifyVCProof = async ({
  vc,
  documentLoader,
  getSuite,
...
type VerifyVCIssuanceResponse =
  | VerifyVCIssuanceResponseSuccess
  | VerifyVCIssuanceResponseFailure;
...
type VerifyVPProofResponse =
  | VerifyVPProofResponseSuccess
  | VerifyVPProofResponseFailure;
...
const verifyVPProof: VerifyVPProof = async ({
  vp,
  documentLoader,
  getSuite,
  getProofPurposeOptions,
...
type VerifyVPIssuanceResponse =
  | VerifyVPIssuanceResponseSuccess
  | VerifyVPIssuanceResponseFailure;
...
export type VerifyVCResponse<VCType extends VC> =
  | VerifyVCResponseSuccess<VCType>
  | VerifyVCResponseFailure;
...
export const verifyVC = async <VCType extends VC>({
  ...
  const validate =
    ajv.getSchema<VCType>(schemaKey) ||
    ajv.addSchema(schema, schemaKey).getSchema<VCType>(schemaKey)!;
  ...
  const proofResult = await verifyVCProof({
    vc,
    documentLoader,
    getSuite,
    getProofPurposeOptions,
...
export type VerifyVPResponse<VPType extends VP> =
  | VerifyVPResponseSuccess<VPType>
  | VerifyVPResponseFailure;
...
export const verifyVP = async <VPType extends VP = VP>({
  ...
  const validate =
    ajv.getSchema<VPType>(schemaKey) ||
    ajv.addSchema(schema, schemaKey).getSchema<VPType>(schemaKey)!;
  ...
  const proofResult = await verifyVPProof({
    vp,
    documentLoader,
    getSuite,
    getProofPurposeOptions,
