import { WalletPersonV2, VCWalletPersonV2Type, getVCWalletPersonV2Context } from '../../wallet/v2'
import { expandVCV2 } from '../__fixtures__'

describe('VCWalletPersonV2', () => {
  it('expands correctly', async () => {
    const expanded = await expandVCV2<VCWalletPersonV2Type, WalletPersonV2>({
      type: 'WalletCredentialPersonV2',
      data: {
        '@type': 'WalletPerson',
        hasCryptographicWallet: {
          '@type': 'CryptographicWallet',
          walletAddress: '0x0000',
          blockchain: 'ethereum',
          keyType: 'secp256k1',
        },
      },
      context: getVCWalletPersonV2Context(),
    })

    expect(expanded).toMatchInlineSnapshot(`
      Object {
        "@id": "urn:uuid:9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d",
        "@type": Array [
          "https://www.w3.org/2018/credentials#VerifiableCredential",
          "https://schema.bloom.co/WalletCredentialPersonV2",
        ],
        "https://www.w3.org/2018/credentials#credentialSubject": Array [
          Object {
            "https://schema.bloom.co/data": Array [
              Object {
                "@type": Array [
                  "https://schema.bloom.co/WalletPerson",
                ],
                "https://schema.bloom.co/hasCryptographicWallet": Array [
                  Object {
                    "@type": Array [
                      "https://schema.bloom.co/CryptographicWallet",
                    ],
                    "https://schema.bloom.co/blockchain": Array [
                      Object {
                        "@value": "ethereum",
                      },
                    ],
                    "https://schema.bloom.co/keyType": Array [
                      Object {
                        "@value": "secp256k1",
                      },
                    ],
                    "https://schema.bloom.co/walletAddress": Array [
                      Object {
                        "@value": "0x0000",
                      },
                    ],
                  },
                ],
              },
            ],
          },
        ],
        "https://www.w3.org/2018/credentials#holder": Array [
          Object {
            "@id": "did:example:123",
          },
        ],
        "https://www.w3.org/2018/credentials#issuanceDate": Array [
          Object {
            "@type": "http://www.w3.org/2001/XMLSchema#dateTime",
            "@value": "2021-05-20T00:00:00.000Z",
          },
        ],
        "https://www.w3.org/2018/credentials#issuer": Array [
          Object {
            "@id": "did:elem:EiD73F4C1VzN7w-bSUN26nRPOh0ohVSkPR0Lv2aybnkqZQ;elem:initial-state=",
          },
        ],
      }
    `)
  })
})
