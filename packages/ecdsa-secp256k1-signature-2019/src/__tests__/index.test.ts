...
describe('EcdsaSecp256k1Signature2019', () => {
  describe('signs a document', () => {
    ...
    describe('throws', () => {
      test('when no signer is provided', () => {
        const suite = new EcdsaSecp256k1Signature2019({
          date: '2021-04-20T00:00:00Z',
        });
        ...
        expect(
          ...
          `"A signer API has not been specified."`
        );
      });

      test('when key has no publicKeyBase58', () => {
        const suite = new EcdsaSecp256k1Signature2019({
      ...
      test('when document is missing the suite and addSuiteContext is false', () => {
        const suite = new EcdsaSecp256k1Signature2019({
          key: EcdsaSecp256k1VerificationKey2019.from(privateKeyPair),
          date: '2021-04-20T00:00:00Z',
        });
  ...
  describe('verifies a document', () => {
    ...
    test('with a verifier', async () => {
      const suite = new EcdsaSecp256k1Signature2019({
        verifier: EcdsaSecp256k1VerificationKey2019.from(
          publicKeyPair
        ).verifier(),
      });
