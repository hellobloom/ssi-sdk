...
describe('resolve', () => {
  describe('resolverRegistry', () => {
    ...
    test('resolves a valid legacy element DID', async () => {
      ...
      const { didDocument, didDocumentMetadata, didResolutionMetadata } = await resolver.resolve(profile.did)
    ...
    describe('returns a null document when', () => {
      test("the given DID doesn't have an initial state", async () => {
        const { didDocument, didDocumentMetadata, didResolutionMetadata } = await resolver.resolve(profiles['12Word'].didDocument.id)
