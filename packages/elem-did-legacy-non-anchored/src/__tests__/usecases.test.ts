import { DocumentLoader, GetProofPurposeOptionsFn, GetSuiteFn, signVC, verifyVC } from '@bloomprotocol/vc'
...
const getSuite: GetSuiteFn = ({ verificationMethod, controller, proofType }) => {
  switch (proofType) {
    case 'EcdsaSecp256k1Signature2019':
      if (controller.startsWith('did:elem') && controller.indexOf('elem:initial-state') >= 0) {
        ...
        if (!publicKey) throw new Error(`Could not find matching publicKey for verificationMethod: ${verificationMethod}`)
...
const getProofPurposeOptions: GetProofPurposeOptionsFn = ({ controller, proofPurpose }) => {
  switch (proofPurpose) {
